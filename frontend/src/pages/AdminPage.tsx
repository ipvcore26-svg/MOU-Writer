...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen bg-dark-900 flex flex-col">
      {/* Header */}
      <header className="bg-dark-800 border-b border-dark-500 px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link to="/" className="text-gray-500 hover:text-gold-400 transition-colors">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
          </Link>
          <div>
            <h1 className="font-serif text-gold-400 text-lg font-semibold">Admin Panel</h1>
            <p className="text-gray-500 text-xs">Template Management</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-gold-500 animate-pulse" />
          <span className="text-xs text-gray-400">IPV Ultra System</span>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-8 py-10 space-y-8">
        ...
        <div className="section-card">
          ...
          {info === null ? (
            ...
            <div className="flex items-start gap-4">
              <div className="w-12 h-14 bg-gold-500/10 border border-gold-500/30 rounded flex items-center justify-center flex-shrink-0">
                <svg className="w-6 h-6 text-gold-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              </div>
              <div>
                <p className="text-sm font-medium text-gray-200">current-template.docx</p>
                <p className="text-xs text-gray-500 mt-1">
                  Last updated: {info.uploadedAt ? new Date(info.uploadedAt).toLocaleString() : '—'}
                </p>
                <p className="text-xs text-gray-500">Size: {formatSize(info.size)}</p>
        ...
        <div className="section-card">
          ...
          <p className="text-xs text-gray-500 mb-4">
            Upload a <code className="text-gold-500 bg-dark-700 px-1 py-0.5 rounded">.docx</code> file with <code className="text-gold-500 bg-dark-700 px-1 py-0.5 rounded">{'{{field1}}'}</code> through <code className="text-gold-500 bg-dark-700 px-1 py-0.5 rounded">{'{{field48}}'}</code> placeholders. The previous template will be replaced automatically.
          </p>
          ...
          <form onSubmit={handleUpload} className="space-y-4">
            <div>
              ...
              <div
                className="border-2 border-dashed border-dark-400 hover:border-gold-500/50 rounded-lg p-8 text-center cursor-pointer transition-colors"
            ...
            {message && (
              <div className="flex items-center gap-2 text-gold-300 text-sm bg-gold-900/20 border border-gold-500/30 px-3 py-2 rounded">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-dark-500">
                  <th className="text-left py-2 text-gold-600 font-medium">Placeholder</th>
                  <th className="text-left py-2 text-gold-600 font-medium">Field</th>
                  <th className="text-left py-2 text-gold-600 font-medium">Section</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={ph} className="border-b border-dark-600 hover:bg-dark-700 transition-colors">
                    <td className="py-1.5 pr-4 font-mono text-gold-500">{ph}</td>
                    <td className="py-1.5 pr-4 text-gray-300">{label}</td>
                    <td className="py-1.5 text-gray-500">{sec}</td>
                  </tr>
                ))}
              </tbody>
            </table>
