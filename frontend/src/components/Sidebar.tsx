...
export default function Sidebar({ activeSection, onSectionClick, watch }: SidebarProps) {
  ...
  return (
    <aside className="w-72 bg-dark-800 border-r border-dark-500 flex flex-col h-screen sticky top-0">
      {/* Logo */}
      <div className="p-6 border-b border-dark-500">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-gold-500 rounded-sm flex items-center justify-center">
            <span className="text-dark-900 font-serif font-bold text-sm">IPV</span>
          </div>
          <div>
            <h1 className="font-serif text-gold-400 text-sm font-semibold leading-tight">IPV Ultra</h1>
            <p className="text-gray-500 text-xs">Term Sheet Generator</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-4 border-b border-dark-500">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-xs font-semibold text-gold-400">{progress}%</span>
        </div>
        ...
        <p className="text-xs text-gray-500 mt-1">{filledFields} / {totalFields} fields</p>
      </div>
            ...
            <button
              ...
              className={`w-full text-left px-3 py-2.5 rounded-md mb-1 flex items-center justify-between transition-all duration-150 group
                ${activeSection === section.id
                  ? 'bg-dark-600 border border-gold-500/30 text-gold-400'
                  : 'text-gray-400 hover:bg-dark-700 hover:text-gray-200'
                }`}
            >
              <div className="flex items-center gap-2.5">
                <div className={`w-2 h-2 rounded-full flex-shrink-0 ${
                  complete ? 'bg-gold-500' : partial ? 'bg-gold-700' : 'bg-dark-400'
                }`} />
                <span className="text-xs font-medium">{section.label}</span>
              </div>
              <span className={`text-xs ${complete ? 'text-gold-500' : 'text-gray-600'}`}>
                {filled}/{total}
              </span>
            </button>
          );
